...
/**
 * Closes an encounter from the Treatment Room.
 * Ported from App\Actions\Encounter\CloseEncounterFromTreatmentRoomAction.
 *
 ...
 */
export default class CloseEncounterFromTreatmentRoomAction {
  ...
  async handle(
    ...
    await db.transaction(async (trx) => {
      ...
      // 1. Complete any open queue transition.
      ...
      // 2. Close the treatment room stage log.
      ...
      if (this.encounterBilling) {
        ...
        const user = await User.find(nurseId, { client: trx })
        await this.encounterBilling.billEncounter(encounter, user, {
          include_treatment_room_fee: true,
        })
      }
