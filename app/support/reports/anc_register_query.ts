...
/**
 * Antenatal Care register — female screening visits with obstetric / pregnancy signals.
 * Ported from App\Support\Reports\AncRegisterQuery.
 */
export class AncRegisterQuery {
  static build(filters: ReportFilters = {}) {
    const query = ScreeningRecord.query()
      .whereHas('patient', (q) => {
        q.where((gender) => {
          gender
      ...
      .where((q) => {
        q.where('currently_pregnant', true)
          .orWhereNotNull('gravida')
          .orWhereNotNull('last_menstrual_period')
          .orWhereNotNull('expected_delivery_date')
      })
