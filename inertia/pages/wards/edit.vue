...
const form = useForm({
  ...
  wing: props.ward.wing,
  location: props.ward.location ?? '',
  notes: props.ward.notes ?? '',
  is_active: props.ward.isActive,
})

function submit() {
  form.put(`/wards/${props.ward.id}`)
}
</script>

<template>
  <StaffLayout>
    <template #header><h1 class="text-lg font-semibold">Edit Ward</h1></template>
