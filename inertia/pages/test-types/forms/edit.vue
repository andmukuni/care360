...
const isSystem = props.form.isSystem
...
function submit() {
  form.put(`/test-types/forms/${props.form.id}`)
}
</script>

<template>
  <StaffLayout>
    <template #header><h1 class="text-lg font-semibold">Edit Result Form</h1></template>

    <form class="max-w-3xl space-y-6" @submit.prevent="submit">
      <div class="space-y-4 theme-panel rounded-lg p-6">
        <div v-if="isSystem" class="rounded bg-amber-50 px-3 py-2 text-sm text-amber-800">
          This is a system form — key, template and custom fields are locked.
        </div>
        <div class="grid grid-cols-2 gap-4">
          <div>
            <label class="block text-sm font-medium mb-1">Key</label>
            <input v-model="form.key" type="text" :disabled="isSystem" class="w-full rounded border border-sand-6 px-3 py-2 disabled:bg-sand-3" />
      ...
      <div v-if="!isSystem" class="space-y-3 theme-panel rounded-lg p-6">
