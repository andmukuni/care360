...
const props = defineProps<{
  testType: {
    ...
    labSpecimenTypeId: number | null
    labResultFormId: number | null
    isActive: boolean
  }
...
const form = useForm({
  ...
  lab_specimen_type_id: props.testType.labSpecimenTypeId,
  lab_result_form_id: props.testType.labResultFormId,
  description: props.testType.description ?? '',
  is_active: props.testType.isActive,
})

function submit() {
  form.put(`/test-types/${props.testType.id}`)
}
</script>

<template>
  <StaffLayout>
    <template #header><h1 class="text-lg font-semibold">Edit Test Type</h1></template>
