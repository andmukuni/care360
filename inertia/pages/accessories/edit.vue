...
const form = useForm({
  accessory_type_id: props.accessory.accessoryTypeId,
  asset_tag: props.accessory.assetTag ?? '',
  name: props.accessory.name ?? '',
  bed_id: props.accessory.bedId,
  status: props.accessory.status,
  notes: props.accessory.notes ?? '',
})

function submit() {
  form.put(`/accessories/${props.accessory.id}`)
}
</script>

<template>
  <StaffLayout>
    <template #header><h1 class="text-lg font-semibold">Edit Accessory</h1></template>
